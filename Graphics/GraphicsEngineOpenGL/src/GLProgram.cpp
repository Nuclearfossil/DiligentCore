...
namespace Diligent
{
    ...
    void GLProgram::InitResources(const SHADER_VARIABLE_TYPE DefaultVariableType, const ShaderVariableDesc *VariableDesc, Uint32 NumVars, IObject *pOwner)
    {
        GLuint GLProgram = static_cast<GLuint>(*this);
        m_AllResources.LoadUniforms(GLProgram, DefaultVariableType, VariableDesc, NumVars);

        SHADER_VARIABLE_TYPE VarTypes[] = {SHADER_VARIABLE_TYPE_STATIC};
        m_ConstantResources.Clone(m_AllResources, VarTypes, _countof(VarTypes), pOwner);
    }
