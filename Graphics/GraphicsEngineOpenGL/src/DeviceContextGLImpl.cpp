...
namespace Diligent
{
    ...
    void DeviceContextGLImpl::CommitShaderResources(IShaderResourceBinding *pShaderResourceBinding)
    {
        if(!DeviceContextBase::CommitShaderResources(pShaderResourceBinding, 0))
            return;
...
#define LOG_MISSING_BINDING(VarType, VarName) LOG_ERROR_MESSAGE( "No ", VarType, " is bound to \"", VarName, "\" variable in shader \"", pShaderGL->GetDesc().Name, "\"" );
    ...
    void DeviceContextGLImpl::Draw( DrawAttribs &DrawAttribs )
    {
        ...
        if( m_pPipelineState->GetDesc().GraphicsPipeline.InputLayout.NumElements > 0 )
        {
            ...
            const auto& VAO = VAOCache.GetVAO( m_pPipelineState.RawPtr(), spIndexBuff, m_VertexStreams, m_NumVertexStreams, m_ContextState );
