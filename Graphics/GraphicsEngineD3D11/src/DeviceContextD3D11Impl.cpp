...
#define DEFINE_D3D11CTX_FUNC_POINTERS(ArrayName, FuncName) \
    ...
    void DeviceContextD3D11Impl::CommitShaderResources(IShaderResourceBinding *pShaderResourceBinding)
    {
        if( !DeviceContextBase::CommitShaderResources(pShaderResourceBinding, 0 /*Dummy*/) )
            return;
...
#ifdef _DEBUG
        auto ppShaders = pPipelineStateD3D11->GetShaders();
#endif
        ...
        for (Uint32 s = 0; s < NumShaders; ++s)
        {
            ...
            if(NumCBs)
            {
                ...
                m_NumCommittedCBs[ShaderTypeInd] = std::max(m_NumCommittedCBs[ShaderTypeInd], NumCBs);
            ...
            if(NumSRVs)
            {
                ...
                m_NumCommittedSRVs[ShaderTypeInd] = std::max(m_NumCommittedSRVs[ShaderTypeInd], NumSRVs);
                UINT MinSlot = UINT_MAX;
                UINT MaxSlot = 0;
            ...
            if(NumSamplers)
            {
                ...
                m_NumCommittedSamplers[ShaderTypeInd] = std::max(m_NumCommittedSamplers[ShaderTypeInd], NumSamplers);
            ...
            if(NumUAVs)
            {
                ...
                m_NumCommittedUAVs[ShaderTypeInd] = std::max(m_NumCommittedUAVs[ShaderTypeInd], NumUAVs);
                UINT MinSlot = UINT_MAX;
                UINT MaxSlot = 0;
...
#ifdef VERIFY_SHADER_BINDINGS
            if( m_DebugFlags & (Uint32)EngineD3D11DebugFlags::VerifyCommittedShaderResources )
            {
                // Use full resource layout to verify that all required resources are committed
                pShaderD3D11->GetResourceLayout().dbgVerifyCommittedResources(
    ...
    void DeviceContextD3D11Impl::Draw( DrawAttribs &DrawAttribs )
    {
        ...
        auto D3D11Topology = TopologyToD3D11Topology( DrawAttribs.Topology );
        if(m_CommittedPrimitiveTopology != D3D11Topology)
        {
            m_pd3d11DeviceContext->IASetPrimitiveTopology( D3D11Topology );
            m_CommittedPrimitiveTopology = D3D11Topology;
        }
