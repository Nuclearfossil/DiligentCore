...
namespace Diligent
{
    void ShaderResourceCacheD3D11::Initialize(const ShaderResourceLayoutD3D11 &Layout, IMemoryAllocator &MemAllocator)
    {
        if (IsInitialized())
        {
        ...
        auto CBCount = Layout.GetMaxCBBindPoint()+1;
        ...
        auto SRVCount = Layout.GetMaxSRVBindPoint()+1;
        ...
        auto SamplerCount = Layout.GetMaxSamplerBindPoint()+1;
        ...
        auto UAVCount = Layout.GetMaxUAVBindPoint()+1;
        ...
        VERIFY_EXPR(GetCBCount()      == CBCount);
        VERIFY_EXPR(GetSRVCount()     == SRVCount);
        VERIFY_EXPR(GetSamplerCount() == SamplerCount);
        VERIFY_EXPR(GetUAVCount()     == UAVCount);
