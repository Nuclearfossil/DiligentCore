...
RenderDeviceD3D11Impl :: RenderDeviceD3D11Impl(IMemoryAllocator &RawMemAllocator, const EngineD3D11Attribs& EngineAttribs, ID3D11Device *pd3d11Device) : 
    TRenderDeviceBase(RawMemAllocator, sizeof(TextureBaseD3D11), sizeof(TextureViewD3D11Impl), sizeof(BufferD3D11Impl), sizeof(BufferViewD3D11Impl), sizeof(ShaderD3D11Impl), sizeof(SamplerD3D11Impl), sizeof(PipelineStateD3D11Impl), sizeof(ShaderResourceBindingD3D11Impl)),
...
void RenderDeviceD3D11Impl::CreateDeferredContext(IDeviceContext **ppCtx)
{
    VERIFY( ppCtx != nullptr, "Null pointer provided" );
    if(!ppCtx)
        return;

    VERIFY( *ppCtx == nullptr, "Overwriting reference to existing object may cause memory leaks" );

    CComPtr<ID3D11DeviceContext> pd3d11DeferredCtx;
    HRESULT hr = m_pd3d11Device->CreateDeferredContext(0, &pd3d11DeferredCtx);
    if(FAILED(hr))
    {
        LOG_ERROR("Failed to create D3D11 deferred context")
        *ppCtx = nullptr;
    }

    auto pImmediateCtx = m_wpImmediateContext.Lock();
    if (!pImmediateCtx)
    {
        LOG_ERROR("Immediate context was destroyed")
        return;
    }
    
    auto pSwapChain = ValidatedCast<DeviceContextD3D11Impl>(pImmediateCtx.RawPtr())->GetSwapChain();
    
    auto &RawAlloctor = GetRawAllocator();
    RefCntAutoPtr<DeviceContextD3D11Impl> pDeviceContextD3D11( NEW(RawAlloctor, "DeviceContextD3D11Impl instance", DeviceContextD3D11Impl, this, pd3d11DeferredCtx, m_EngineAttribs, true) );
    pDeviceContextD3D11->SetSwapChain(pSwapChain);

    pDeviceContextD3D11->QueryInterface(IID_DeviceContext, reinterpret_cast<IObject**>(ppCtx) );
}
