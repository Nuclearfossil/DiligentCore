...
public:
    ...
    void Initialize(const class ShaderResourceLayoutD3D11 &Layout, class IMemoryAllocator &MemAllocator);
    ...
    static __forceinline Uint8 UnpackCBCount       (Uint32 PackedCounts) {return (PackedCounts >>   0) & 0xFF; }
    static __forceinline Uint8 UnpackSRVCount      (Uint32 PackedCounts) {return (PackedCounts >>   8) & 0xFF; }
    static __forceinline Uint8 UnpackSamplerCount  (Uint32 PackedCounts) {return (PackedCounts >>  16) & 0xFF; }
    static __forceinline Uint8 UnpackUAVCount      (Uint32 PackedCounts) {return (PackedCounts >>  24) & 0xFF; }

    __forceinline Uint8 GetCBCount()     const{ return UnpackCBCount     (m_ResourceCounts);}
    __forceinline Uint8 GetSRVCount()    const{ return UnpackSRVCount    (m_ResourceCounts);}
    __forceinline Uint8 GetSamplerCount()const{ return UnpackSamplerCount(m_ResourceCounts);}
    __forceinline Uint8 GetUAVCount()    const{ return UnpackUAVCount    (m_ResourceCounts);}
