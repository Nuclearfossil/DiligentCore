...
public:
    ...
    void DiscardUsedPages(Uint64 FenceValue);
    DynamicAllocation AllocateUploadBuffer(size_t SizeInBytes, size_t Alignment = DEFAULT_ALIGN){return m_CpuLinearAllocator.Allocate(SizeInBytes, Alignment);}

    void DiscardUsedDescriptorHeaps(Uint64 FenceValue);
    DescriptorHeapAllocation AllocateGPUVisibleDescriptor( D3D12_DESCRIPTOR_HEAP_TYPE Type, UINT Count = 1 );
...
protected:
    ...
    std::vector< RefCntAutoPtr<IDeviceObject> >  m_PendingBarrierObjects;
    ...
    RefCntAutoPtr<DynamicDescriptorHeapAllocator> m_pDynamicGPUDescriptorAllocator[D3D12_DESCRIPTOR_HEAP_TYPE_NUM_TYPES];
...
{
    m_pCommandList->SetDescriptorHeaps(HeapCount, HeapPtrs);
