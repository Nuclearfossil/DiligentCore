    ...
    TTextureBase(TexObjAllocator, TexViewObjAllocator, pRenderDeviceD3D12, TexDesc)
{
	...
	Desc.Alignment = 0;
    if(m_Desc.Type == RESOURCE_DIM_TEX_1D_ARRAY || m_Desc.Type == RESOURCE_DIM_TEX_2D_ARRAY || m_Desc.Type == RESOURCE_DIM_TEX_CUBE_ARRAY)
    ...
    if(bInitializeTexture)
    {
        if( InitData.NumSubresources != static_cast<Uint32>(Desc.MipLevels) * static_cast<Uint32>(Desc.DepthOrArraySize) )
            LOG_ERROR_AND_THROW("Incorrect number of subresources in init data. ", Desc.MipLevels * Desc.DepthOrArraySize, " expected, while ", InitData.NumSubresources, " provided")
        ...
        std::vector<D3D12_SUBRESOURCE_DATA> D3D12SubResData(InitData.NumSubresources);
...
void TextureD3D12Impl::Unmap( IDeviceContext *pContext )
{
